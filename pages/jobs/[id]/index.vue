  ...
  <v-container v-if="!pending">
    ...
    <v-row justify="center" align="center">
      ...
      <v-col v-if="opportunity.pdf">
        <v-btn size="x-large" :to="`/jobs/${opportunity.id}/pdf/${replaceString(opportunity.pdfPath!)}`" @click="setPdf(opportunity.pdf)">View
          Attached File</v-btn>
      </v-col>
    </v-row>
  </v-container>
...
const { data, pending } = useDocument(doc(collection(db, 'jobs'), `${route.params.id}`))
