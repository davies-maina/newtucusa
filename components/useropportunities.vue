  ...
  <div>
    <v-row justify="center">
      <v-col class="text-center">
        ...
        <p v-if="cdfbanner">
          If CDF is selected, CDF results will be explicitly for your ward ({{ home }})
        </p>
      </v-col>
    </v-row>
    <v-row v-if="opportunities!.length > 0">
      <TransitionGroup name="list">
        <v-col v-for="opportunity in opportunities" :key="opportunity.id" cols="auto">
          <v-card class="mx-auto my-8" max-width="344" min-width="300" elevation="16" min-height="250">
            ...
            <v-card-text v-else>
              {{ sliceString(plaintext(opportunity.desc)) }}... <NuxtLink
                :to="`${name?.toString()}/${opportunity.id}`"
                class="text-primary"
              >
                Read more
              </NuxtLink>
